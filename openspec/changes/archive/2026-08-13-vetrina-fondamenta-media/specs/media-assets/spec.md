...
**Date**: 2026-08-11
**Status**: Draft
**Tipo**: NUOVA spec (il dominio non esiste in `openspec/specs/`)
...
**Fuori scope in questa fase**: l'API pubblica `/api/public/*` e qualunque esposizione a
visitatori anonimi (Fase 2). In questa fase i media esistono, sono amministrabili e sono
serviti dal web server, ma **nessun endpoint pubblico li elenca**.

**Stato verificato del codice prima della change**: nessuna entità `MediaAsset`, nessun
...
In questa fase `Cartella` MUST essere una **stringa libera** non nulla con default
`"generale"`: il sistema MUST NOT imporre un insieme chiuso di valori. L'insieme chiuso è
una decisione della Fase 2, quando si sapranno le cartelle vere del sito; l'indice di
ordinamento per cartella è già adatto a entrambe le forme e la transizione non richiede una
migrazione di dati.
...
Il sistema MUST **rifiutare** l'eliminazione di un `MediaAsset` referenziato da almeno un
prodotto: il tentativo MUST produrre un errore esplicito il cui messaggio **nomina i
prodotti** che lo utilizzano, e MUST NOT eliminare né il record né alcun file. Il requisito
è espresso in termini di comportamento e non di codice di stato HTTP, perché l'operazione
non è più un `DELETE` REST ma una mutation GraphQL: ciò che MUST valere è il rifiuto, la
diagnosticità del messaggio e l'assenza di qualunque effetto.
...
possa lasciare prodotti con un riferimento pendente.
