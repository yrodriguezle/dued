...
- [ ] `dotnet build` e `dotnet test` passano; `npm run ts:check`, `npm run lint` e `npm run test` passano
- [ ] Le due migrazioni si applicano in sequenza su un database esistente senza perdita di dati
- [ ] Un amministratore carica una foto da `MediaLibrary`, e `ls /opt/duedgusto/media/2026/…` mostra **4 larghezze × 2 formati** più il record in `MediaAsset`
- [ ] `exiftool` sui file generati **non** mostra GPS né alcun metadato EXIF
- [ ] L'immagine è raggiungibile via `https://<host>/media/…` servita da **nginx** (non da .NET) con `expires 1y`
- [ ] Un upload da 15MB non produce un 413 opaco ma un errore leggibile, o passa se il limite è stato alzato
- [ ] Un'immagine oltre la soglia di megapixel viene rifiutata **senza** che la memoria del container esploda
- [ ] `DELETE` su un `MediaAsset` referenziato risponde **409** e non cancella nulla
...
- [ ] 🔴 **Il test del confine passa**: un `mutateProdotto` con payload di sola cassa **non** azzera i campi vetrina del prodotto
- [ ] Un utente autenticato **non amministratore** riceve un errore dal backend su ogni scrittura media/vetrina, anche chiamando GraphQL e REST direttamente (non solo UI nascosta)
- [ ] 🔴 **Simulazione di deploy**: dopo `deploy.sh` i file in `/opt/duedgusto/media` sono ancora tutti lì
- [ ] 🔴 **Simulazione di ripristino**: `backup.sh` produce un archivio che contiene i media, e un restore ricostruisce DB **e** file senza 404
- [ ] Un riavvio del backend con `SEED_ON_STARTUP=true` **non** duplica il menu "Sito"
