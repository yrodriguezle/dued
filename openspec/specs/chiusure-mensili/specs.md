...
> Nota sullo schema GraphQL (corretta in fase di archiviazione): la change ha introdotto
> una modifica **additiva e non-breaking** allo schema — il nuovo field
> `speseGiornaliereRegistriCalcolate` su `ChiusuraMensileType` (esposto anche nel
> fragment e nel tipo TS frontend, e usato nel report di stampa per far quadrare le voci
> con il nuovo netto). Il campo `ricavoNettoCalcolato` resta invariato nel nome e nel
> tipo; cambia la formula con cui viene calcolato a runtime (proprietà `[NotMapped]`,
> nessun dato persistito). Nessuna migrazione database.
...
intenzionale e documentato. La somma delle spese giornaliere dei registri inclusi è
esposta come field GraphQL additivo `speseGiornaliereRegistriCalcolate` su
`ChiusuraMensileType`. Lista, dettaglio e report di stampa
...
registri) in modo che il dettaglio sia coerente con il nuovo netto.
