...
function FormToolbar({
  ...
  const formLocked = useMemo(
    () =>
      isFormLocked !== undefined
        ? isFormLocked
        : formikProps?.status?.isFormLocked,
    [formikProps?.status?.isFormLocked, isFormLocked]
  );
  const formStatus = useMemo(
    () => formStatusAsProp || formikProps?.status?.formStatus,
    [formStatusAsProp, formikProps?.status?.formStatus]
  );
  const formHasChanged = useMemo(() => {
    const valuesToCompare =
      typeof entity === "object" && entity !== null ? entity : initialValues;
    const hasChanged = !isEqual(valuesToCompare, values);
    return hasChanged;
  }, [entity, initialValues, values]);
  const disabledSave = useMemo(
    () =>
      disabledSaveProp !== undefined
        ? disabledSaveProp || formLocked || isSubmitting
        : !formHasChanged || formLocked || isSubmitting,
