...
function FormToolbar({
  isFormLocked,
  formStatus: formStatusAsProp,
  disabledSave: disabledSaveProp,
  entity,
  getFormikContext,
  setMessage,
  onFormChanged,
  ...
  const {
    values, status, initialValues, isSubmitting,
  } = formikProps;
  ...
  const disabledSave = useMemo(() => ((disabledSaveProp !== undefined)
    ? (disabledSaveProp || formLocked || isSubmitting)
    : (!formHasChanged || formLocked || isSubmitting)), [disabledSaveProp, formHasChanged, formLocked, isSubmitting]);
  ...
  const handleSubmitAndFetchUpdatedEntity = useCallback(
    async () => {
      if (formikProps) {
        const {
        ...
        const isValid = onValidateView
          ? await onValidateView(
            () => validateFormikForm(formikProps.values, validateForm, setTouched, setMessage),
          )
      ...
      <Toolbar variant="dense" className="header-bar">
        <Stack direction="row" spacing={2}>
          ...
          <Button
            ...
            onClick={handleSubmitAndFetchUpdatedEntity}
          >
            Salva
          </Button>
          <Button
