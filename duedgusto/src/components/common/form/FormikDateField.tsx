...
import DateField, { DateFieldProps, DateFieldRef } from "./DateField";
...
interface FormikDateFieldProps<Values> extends Omit<DateFieldProps, "onChange" | "onBlur"> {
  name: string;
  onChange?: (name: string, value: string, field: FieldInputProps<string>, form: FormikProps<Values>) => void;
  onBlur?: (event: React.FocusEvent<HTMLInputElement | HTMLTextAreaElement, Element>, field: FieldInputProps<string>, form: FormikProps<Values>) => void;
  params?: {
...
function FormikDateField<Values>({ name, onChange, onBlur, params, ...props }: FormikDateFieldProps<Values>, ref: Ref<DateFieldRef>) {
  return (
    <FastField
      name={name}
      params={params}
...
export default forwardRef(FormikDateField);
