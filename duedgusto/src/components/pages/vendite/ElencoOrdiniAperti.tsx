...
interface ElencoOrdiniApertiProps {
  /**
   * Se l'elenco è a schermo. È l'interruttore dello `skip` della query: da dentro un cassetto
   * chiuso non si interroga il server, da una pagina vale sempre `true`.
   */
  attivo: boolean;
  /**
  ...
  /** Assente, l'azione «riprendi» non compare. */
...
/**
 * Il **corpo** dell'elenco degli ordini aperti, con le due uscite su ogni riga: incassare o
 * annullare. Senza guscio: lo montano il cassetto (`OrdiniAperti`) e la pagina (`Ordini`).
 *
 * <p>🔴 <b>Perché è un componente a sé.</b> Finché l'elenco viveva dentro un `Drawer`, montarlo
 * come pagina avrebbe significato una modale sempre aperta e non chiudibile — con un pulsante
 * «Chiudi» che non ha un chiamante a cui tornare. Il guscio è ciò che cambia fra i due usi; la
 * query, le due mutation e i due dialog di conferma no.</p>
 *
 ...
 */
function ElencoOrdiniAperti({ attivo, registroCassaId, onRiprendi, onRisolto, titolo, descrizione }: ElencoOrdiniApertiProps) {
  ...
  return (
    <>
      <Box sx={{ p: 2, maxWidth: 640, mx: "auto", width: "100%", display: "flex", flexDirection: "column", minHeight: 0, flex: 1 }}>
          <Typography
            variant="h6"
            gutterBottom
          >
...
export default ElencoOrdiniAperti;
