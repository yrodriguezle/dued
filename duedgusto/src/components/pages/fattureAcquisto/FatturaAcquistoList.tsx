...
import { fatturaAcquistoFragment } from "../../../graphql/fornitori/fragments";
...
import { mutationDeleteFatturaAcquisto } from "../../../graphql/fornitori/mutations";

type FatturaAcquistoNonNull = Exclude<FatturaAcquisto, null>;

function FatturaAcquistoList() {
  const navigate = useNavigate();
  const { setTitle } = useContext(PageTitleContext);
  const gridRef = useRef<GridReadyEvent<DatagridData<FatturaAcquistoNonNull>> | null>(null);
  const [selectedRows, setSelectedRows] = useState<DatagridData<FatturaAcquistoNonNull>[]>([]);
  const onConfirm = useConfirm();

  const [deleteInvoice] = useMutation(mutationDeleteFatturaAcquisto);

  useEffect(() => {
    setTitle("Fatture Acquisto");
  }, [setTitle]);

  const { data: invoices, refetch } = useGetAll<FatturaAcquistoNonNull>({
    fragment: fatturaAcquistoFragment,
    queryName: "fattureAcquisto",
    fragmentBody: "...FatturaAcquistoFragment",
    fetchPolicy: "network-only",
  });

  const handleGridReady = useCallback((event: GridReadyEvent<DatagridData<FatturaAcquistoNonNull>>) => {
  ...
  const handleNew = useCallback(() => {
    navigate("/gestionale/fatture-acquisto-details");
  }, [navigate]);
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      await Promise.all(selectedRows.map((row) => deleteInvoice({ variables: { fatturaId: row.fatturaId } })));
  ...
  const handleRowDoubleClick = useCallback(
    (event: DatagridRowDoubleClickedEvent<FatturaAcquistoNonNull>) => {
      if (event.data) {
        navigate(`/gestionale/fatture-acquisto-details?invoiceId=${event.data.fatturaId}`);
      }
    },
    [navigate]
  );

  const columnDefs = useMemo<DatagridColDef<FatturaAcquistoNonNull>[]>(
    () => [
      {
        headerName: "ID",
        field: "fatturaId",
...
export default FatturaAcquistoList;
